	...
	if (m.key) {
		...
		if (m.isGroup) {
			...
			if (metadata.addressingMode === 'lid') {
				const participant = metadata.participants.find(a => a.id === m.sender)
				...
				store.contacts[m.sender] = { ...store.contacts[m.sender], id: jidNormalizedUser(m.fromMe && naze.user.lid || participant?.id || m.sender), phoneNumber: jidNormalizedUser(m.fromMe && naze.user.id || participant?.phoneNumber || m.sender), name: m.pushName };
			}
